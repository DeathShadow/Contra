...
return '.var_export($this->list, true).';'.chr(10).'?>';
	...
	public function addCmd($user, $cmd) {
		...
		if($over!==false) {
			...
			if(in_array(strtolower($cmd),$over['ban']))
				$this->list['override'][$over['user']]['ban'] =
					array_del_key($over['ban'],array_search(strtolower($cmd),$over['ban']));
			$this->list['override'][$user]['allow'][] = strtolower($cmd);
		} else {
	...
	public function banCmd($user, $cmd) {
		...
		if($over!==false) {
			...
			if(in_array(strtolower($cmd),$over['allow']))
				$this->list['override'][$over['user']]['allow'] =
					array_del_key($over['allow'],array_search(strtolower($cmd),$over['allow']));
			$this->list['override'][$over['user']]['ban'][] = strtolower($cmd);
		} else {
			$this->list['override'][$user] = array(
	...
	public function remCmd($user, $cmd) {
		...
		if($over!==false) {
			...
			if(in_array(strtolower($cmd),$over['ban']))
				$this->list['override'][$over['user']]['ban'] =
					array_del_key($over['ban'],array_search(strtolower($cmd),$over['ban']));
			if(empty($this->list['override'][$over['user']]['allow'])
			&& empty($this->list['override'][$over['user']]['ban']))
				unset($this->list['override'][$over['user']]);
		}
		$this->UpdateList();
		return true;
	}
	public function overrides($user) {
		foreach($this->list['override'] as $suser => $commands) {
	...
	*/
	...
	public function add_class($name, $order) {
		...
		$pcs = $this->list;
		unset($pcs['override']); unset($pcs['pc']);
		$pcs[$order] = array(); krsort($pcs);
		$ovrs = $this->list['override'];
		$names = $this->list['pc']; krsort($names);
		$this->list = $pcs; $this->list['override'] = $ovrs;
		$this->list['pc']=$names;
		$this->UpdateList();
		return true;
	}
