...
class extension {
	...
	final function load() {
		...
		if($this->name !== Null) {
			if($this->author !== Null) {
				if($this->type != EXT_CUSTOM
				&& $this->type != EXT_SYSTEM
				&& $this->type != EXT_LIBRARY) $this->type = EXT_CUSTOM;
				if(!array_key_exists($this->name, $this->Bot->mod)) {
					$this->Bot->mod[$this->name] = $this;
					unset($this->evts); unset($this->cmds);
					if(DEBUG) $this->Console->Notice('Loaded Module '.$this->name.'.');
					return;
					// eh... Houston, we have a problem!
				} else { $this->Console->Warning($file.' tried loading '.$this->name.', which is already loaded.'); }
			} else { $this->Console->Warning($file.' tried loading a module without an author.'); }
		} else { $this->Console->Warning('No name was provided for a loading module in '.$file.', ignoring.'); }
	...
	final protected function Read($file, $format = 0) {
		...
		switch($format) {
			case 2: return include './storage/mod/'.$this->name.'/'.$file.'.bsv';
				break;
			case 1: return file_get_contents('./storage/mod/'.$this->name.'/'.$file.'.bsv');
				break;
			case 0:
			default: return unserialize(file_get_contents('./storage/mod/'.$this->name.'/'.$file.'.bsv'));
				break;
		}
	}
	...
	final protected function Write($file, $data, $format = 0) {
		...
		switch($format) {
			case 2: save_config('./storage/mod/'.$this->name.'/'.$file.'.bsv', $data);
				break;
			case 1: file_put_contents('./storage/mod/'.$this->name.'/'.$file.'.bsv', $data);
				break;
			case 0: default:
