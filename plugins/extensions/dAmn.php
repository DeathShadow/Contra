...
class dAmn_commands extends extension {
	...
	function c_say($ns, $from, $message, $target) {
		$this->dAmn->say($target,args($message, 1, true));
	}
	...
	function c_raw($ns, $from, $message, $target) {
		$this->dAmn->send(str_replace('\n', LBR, args($message, 1, true)));
	}
	...
	function e_ping($ns, $from, $message) {
		...
		if (strtolower($from) != strtolower($this->Bot->username)) {
			return;
		}
		if ($message != $this->ping_who.': Ping?') {
			return;
		}
	...
	function e_kicked($ns, $from, $r = false) {
		if ($r!==false) {
			if (strpos(strtolower($r),'autokicked') !== false || strpos(strtolower($r),'not privileged') !== false) {
				return;
			}
		}
				...
				foreach ($this->dAmn->chat[$chan]['pc'] as $ord => $pc) {
					...
					foreach ($this->dAmn->chat[$chan]['member'] as $name => $data) {
						if ($pc == $data['pc']) {
							...
							substr($name, 0, 1).'<b></b>'.substr($name, 1).($con == 1 ? '' : '['.$con.']').', ';
						}
					}
					if ($usrs !== 0) {
						$body.= '<br/><b>'.$pc.'</b>('.$usrs.')<br/><sub>-> '.rtrim($list, ', ').'</sub>';
					}
				}
	...
	function c_whois($ns, $from, $message, $target) {
		...
		if (!empty($user)) {
			...
			$this->dAmn->say($ns, $from.': Use this command to get whois info on a user.');
		}
	}
	...
	function e_provider($ns) {
		if ($ns == 'chat:DataShare') {
			$this->dAmn->npmsg('chat:datashare', 'BDS:PROVIDER:CAPS:BOTCHECK,BOTCHECK-EXT,CLINK', TRUE);
		}
	}
	...
	function c_disconnects($ns, $from, $message, $target) {
		$this->dAmn->say($ns, $from.': I have been disconnected '.$this->dAmn->disconnects.' '.($this->dAmn->disconnects==1?'time':'times').'.');
	}
