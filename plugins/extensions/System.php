...
class System_commands extends extension {
	...
	function c_eval($ns, $from, $message, $target) {
		...
		if(!function_exists($cmd) || preg_match('/\b(escapeshellarg|escapeshellcmd|exec|passthru|proc_close|proc_get_status|proc_nice|proc_open|proc_terminate|shell_exec|system|rm|mv|shutdown|kill|killall)\b/i',$cmd))
